...
class UIManager : XNAControl, IWindowParentControl
{
    ...
    {
        if (!UserSettings.Instance.EnableMCP)
        {
            Logger.Log("Skipping starting of MCP server because the MCP server is disabled by user preference.");
            return;
        }

        try
        {
            mcpServer = new MCPServer(WindowManager, map, mutationManager, mapUI.MutationTarget, mapUI, UserSettings.Instance.MCPServerPort);
        ...
        catch (Exception ex)
        {
            ...
            AddCallback(() => EditorMessageBox.Show(WindowManager, Translate(this, "FailedToLaunchMCPServer.Header", "Failed to launch MCP server"), 
                string.Format(Translate(this, "FailedToLaunchMCPServer.Description", "Failed to launch MCP server. Returned error was: \"{0}\"." + Environment.NewLine + Environment.NewLine + "MCP server functionality will be unavailable in this session."), ex.Message),
